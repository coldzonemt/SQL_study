...
SELECT DISTINCT CONCAT(m.firstname, ' ', m.surname, ': ', f.name) as tennis_booking
FROM Bookings as b
...
SELECT 
	CONCAT (m.firstname, ' ', m.surname) as guest_name, f.name, 
CASE WHEN m.memid = 0
...
SELECT 
	CONCAT (m.firstname, ' ', m.surname) as guest_name, f.name, 
CASE WHEN m.memid = 0
THEN f.guestcost * b_sb.slots
ELSE f.membercost * b_sb.slots
END AS booking_cost
FROM (
  SELECT * FROM Bookings AS b
	WHERE b.starttime LIKE '2012-09-14%'
) as b_sb
JOIN Facilities as f ON (b_sb.facid = f.facid)
JOIN Members as m ON (b_sb.memid = m.memid)
HAVING booking_cost > 30
ORDER BY booking_cost DESC;
...
SELECT f.name, SUM(b.slots) as number_bookings, MONTH(b.starttime) AS month
FROM Bookings as b
JOIN Facilities AS f ON b.facid = f.facid
WHERE b.memid > 0
